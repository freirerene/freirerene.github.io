const darkSwitch=document.getElementById("darkSwitch");

function initTheme(){
	const e=null!==localStorage.getItem("darkSwitch")&&"dark"===localStorage.getItem("darkSwitch");
	darkSwitch.checked=e,e?document.body.setAttribute("class","dark"):document.body.removeAttribute("class")
	darkSwitch.checked=e,e?(document.querySelector('.theme-icon').src = sun):(document.querySelector('.theme-icon').src = moon);
}
function resetTheme(){
	darkSwitch.checked?(document.body.setAttribute("class","dark"),localStorage.setItem("darkSwitch","dark")):(document.body.removeAttribute("class"),localStorage.removeItem("darkSwitch"))
	darkSwitch.checked?(document.querySelector('.theme-icon').src = sun):(document.querySelector('.theme-icon').src = moon);
	}

window.addEventListener("load",()=>{darkSwitch&&(initTheme(),darkSwitch.addEventListener("change",()=>{resetTheme()}))
});



var sun = "data:image/svg+xml;base64,";

var moon = "data:image/svg+xml;base64,";
